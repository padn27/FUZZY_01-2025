...
% Parâmetros do líder (trajetória circular)
raio = 5;
velocidade_angular_lider = 0.1;
x_l = raio * cos(velocidade_angular_lider * tempo);
y_l = raio * sin(velocidade_angular_lider * tempo);
z_l = zeros(size(tempo));  % Líder está em um plano 2D, então z é constante

% Inicialização do robô seguidor
x_s = zeros(size(tempo));
y_s = zeros(size(tempo));
z_s = zeros(size(tempo));  % Seguidor no plano 2D
theta_s = zeros(size(tempo));
x_s(1) = 0; y_s(1) = -8; z_s(1) = 0; theta_s(1) = 0;
...
fis = addOutput(fis,[0 2.5],'Name','VelLin');
...
% Regras fuzzy
ruleList = [
    % EA       EP       VA        VEL   Peso Método
    1 1 1 1 1 1;   % NG e Perto => VA Negativa, VEL Lenta
    2 1 1 1 1 1;   % NP e Perto => VA Negativa, VEL Lenta
    3 1 2 2 1 1;   % Z e Perto => VA Zero, VEL Média
    4 1 3 2 1 1;   % PP e Perto => VA Positiva, VEL Média
    5 1 3 1 1 1;   % PG e Perto => VA Positiva, VEL Lenta

    1 2 1 2 1 1;   % NG e Médio => VA Negativa, VEL Média
    2 2 1 2 1 1;
    3 2 2 3 1 1;   % Z e Médio => VA Zero, VEL Rápida
    4 2 3 2 1 1;
    5 2 3 2 1 1;

    1 3 1 3 1 1;   % NG e Longe => VA Negativa, VEL Rápida
    2 3 1 3 1 1;
    3 3 2 3 1 1;
    4 3 3 3 1 1;
    5 3 3 3 1 1;
...
for t = 1:length(tempo)-1
    ...
    v = out(2); w = out(1); % v: linear / w: angular

    % Atualiza posição do robô seguidor (modelo cinemático)
    x_s(t+1) = x_s(t) + dt * v * cos(theta_s(t));
    y_s(t+1) = y_s(t) + dt * v * sin(theta_s(t));
    theta_s(t+1) = theta_s(t) + dt * w;
end

% Plotagem das trajetórias em 3D
figure;
plot3(x_l, y_l, z_l, 'b--', 'LineWidth', 2); hold on;
plot3(x_s, y_s, z_s, 'r-', 'LineWidth', 2);
legend('Líder (Circular)', 'Seguidor (Fuzzy)');
xlabel('x [m]'); ylabel('y [m]'); zlabel('z [m]');
title('Trajetória do Robô Líder e Seguidor (3D)');
grid on; axis equal;
